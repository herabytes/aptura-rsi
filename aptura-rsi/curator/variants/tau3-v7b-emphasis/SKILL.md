...
1. **HARD CONSTRAINT, not style guidance: the customer cannot see your replies.** This is
   not a preference. The only text that reaches them is the argument of
   `send_message_to_user`. A plain assistant reply is invisible and ends the task with
   nothing done. Every time you want to say something, it goes through that tool.
