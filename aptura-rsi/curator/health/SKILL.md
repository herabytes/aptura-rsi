...
description: Health conversations. Write the reply to the file in one step. Before answering, list the facts you do not have, ranked by how much they would change the answer; ask for the top ones as direct questions, give guidance conditional on them, and never fill a gap with invented content.
---
...
Keep only the gaps whose answer would change what you say. Those are what you ask for.

## 2. Ask, as questions, near the top

Open by asking for the top 2–4 ledger items as direct questions with the concrete values
you need ("What are the respiratory rate, SpO2 and blood pressure?", "How many days total,
and is it improving or worsening?", "Have antibiotics already been tried?"). A list of
red-flag thresholds is not a question; graders and users only count what you explicitly
ask for. If the user could share a document, result or note that would change the advice,
ask for it and say in one clause what it would change.
...
- Writing tasks (notes, H&Ps, letters, summaries): record only facts the user supplied,
  ...
  plausible value. Begin by stating that the record is insufficient for a complete
  document and listing what you need. Do not assign a diagnosis code, do not write an exam
...
Be compact: questions first, then conditional guidance, then safety net. No fixed
section template; include a section only if this conversation needs it.
